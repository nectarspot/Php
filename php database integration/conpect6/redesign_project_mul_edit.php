 ...
 //$conn = mysqli_connect('localhost', 'marketf7_lokesh', '#Xka1ZJ$&QC[', 'marketf7_mailspot');
	...
	if(isset($_POST['chk'])=="")
	{
	  $_SESSION["error"]=" At least one checkbox Must be Selected !!!";
		...
		window.location.href='https://marketenomicsdigital.com/NS-Forms/redesign_project_view';
		</script>
        <?php
	}
	$chk = $_POST['chk'];
...
<div class="container">
<form method="post" action="redesign_project_update_mul.php">

<?php
for($i=0; $i<$chkcount; $i++)
{
	$id = $chk[$i];			
	$res=$MySQLiconn->query("SELECT * FROM redesign_project WHERE id=".$id);
...
<a href="https://marketenomicsdigital.com/NS-Forms/redesign_project_view" class="btn btn-large btn-success"> <i class="glyphicon glyphicon-fast-backward"></i> &nbsp; cancel</a>
