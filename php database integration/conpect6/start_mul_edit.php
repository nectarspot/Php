 ...
 //$conn = mysqli_connect('localhost', 'marketf7_lokesh', '#Xka1ZJ$&QC[', 'marketf7_mailspot');
	...
	if(isset($_POST['chk'])=="")
	{
	  $_SESSION["error"]=" At least one checkbox Must be Selected !!!";
		...
		window.location.href='https://marketenomicsdigital.com/NS-Forms/start_view';
		</script>
        <?php
	}
	$chk = $_POST['chk'];
...
<div class="container">
<form method="post" action="start_update_mul.php">

<?php
for($i=0; $i<$chkcount; $i++)
{
	$id = $chk[$i];			
	$res=$MySQLiconn->query("SELECT * FROM start WHERE id=".$id);
	...
	<div class="modal-dialog">
        ...
        <div class="col-sm-12">
          ...
          <div class="form-group">
           ...
           <div class="form-group">
            <label for="" style="color:black;">About Us</label>
             <input type="text" class="form-control" name="about_us[]" value="<?php echo $row['about_us']; ?>" />
          </div>
...
<a href="https://marketenomicsdigital.com/NS-Forms/start_view" class="btn btn-large btn-success"> <i class="glyphicon glyphicon-fast-backward"></i> &nbsp; cancel</a>
