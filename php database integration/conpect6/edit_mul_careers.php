...
<?php
 ...
 //$conn = mysqli_connect('localhost', 'marketf7_lokesh', '#Xka1ZJ$&QC[', 'marketf7_mailspot');
	...
	if(isset($_POST['chk'])=="")
	{
		...
		window.location.href='https://marketenomicsdigital.com/NS-Forms/careers_apply_view.php';
		</script>
        <?php
	}
	$chk = $_POST['chk'];
...
<div class="container">
<form method="post" action="update_mul_careers.php">

<?php
for($i=0; $i<$chkcount; $i++)
{
	$id = $chk[$i];			
	$res=$MySQLiconn->query("SELECT * FROM ns_careers_apply WHERE id=".$id);
	...
	<div class="modal-dialog">
        ...
        <div class="col-sm-12">
          ...
          <div class="form-group">
           ...
           <div class="form-group">
            <label for="" style="color:black;">Time To Call</label>
          <input type="text" class="form-control" name="time_to_call[]" value="<?php echo $row['time_to_call']; ?>" />
          </div>
           <div class="form-group">
            <label for="" style="color:black;">Full Time/Part Time</label>
           <input type="text" class="form-control" name="full_part[]" value="<?php echo $row['full_part']; ?>" />
          </div>
           <div class="form-group">
